async function getPessoa(url) {
    const dados = await fetch(url);
    return dados.json();
}

async function imprimirPessoa(url) {
    const p = await getPessoa(url);
    console.log(p);
}

imprimirPessoa('http://localhost/Exemplos-JavaScript-Request/pessoa.json');
